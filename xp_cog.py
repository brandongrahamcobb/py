...
# XP table
XP_TABLE = [
    1, 15, 34, 57, 92, 135, 372, 560, 840, 1242, 1144, 1573, 2144, 2800, 3640, 4700, 5893, 7360, 9144, 11120,
...
class XPCog(commands.Cog):
    ...
    async def add_xp(self, user_id, xp_gain):
        if user_id not in self.user_xp:
            self.user_xp[user_id] = 0

        self.user_xp[user_id] += xp_gain
        new_level = self.get_level(self.user_xp[user_id])
        return new_level
    ...
    async def level(self, ctx):
        ...
        xp = self.user_xp.get(user_id, 0)
        level = self.get_level(xp)
        await ctx.send(f'{ctx.author.mention}, you are at level {level}.')
    ...
    async def on_message(self, message):
        ...
        new_level = await self.add_xp(user_id, xp_gain)

        current_level = self.get_level(self.user_xp[user_id])
