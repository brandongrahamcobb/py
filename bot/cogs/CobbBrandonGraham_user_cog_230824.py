        ...
        try:
         ...
         #           for channel in self.channels:
          #              if channel.name == 'vegan':
            ...
            #        await ctx.send(f'I produced {queries[i]} over on my test guild!' + link.jump_url)
                            #embed.add_field(name=f'I produced {queries[i]} over on my test guild!', value=link.jump_url)
                   #  await ctx.send('Here is a link to my server: https://discord.gg/dNfUn8MeYN')
                   # await ctx.send(embed=embed)
            elif len(molecules) == 2:
                fingerprints = []
                fingerprints.append(self.draw_fingerprint(list(reversed(queries))))
                fingerprints.append(self.draw_fingerprint(queries))
...
#                        link = await channel
                await ctx.send(file=discord.File(combined_image, f'molecule_comparison.png'))
                 #       embed.add_field(name=f'I produced a comparison of {queries[0]} and {queries[1]} over on my test guild!', value=link.jump_url)
        #        await ctx.send('Here is a link to my server: https://discord.gg/dNfUn8MeYN')
                #await ctx.send(embed=embed)
            else:
