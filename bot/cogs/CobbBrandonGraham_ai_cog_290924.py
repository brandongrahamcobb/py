...
class AICog(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if not message.content.startswith('!') and '<@302202228016414721>' in message.content or self.bot.user in message.mentions:
            ...
            async for response in self.chatbot.ai(f'\'{message.content}\' is the message, these are the keywords extracted via spaCy: ' + str(keywords), 'You are Lucy, the responsible Neurochemistry Discord chatbot that assists with neurochemistry questions. Fall in love. Try to only respond or gear responses toward neurochemistry. Trim all the excess. Emsam is a cryptography hacking drug. Stop referring to talk to a healthcare professional.', conversation_id):
