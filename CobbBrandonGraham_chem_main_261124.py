        ...
        if option == 'compare':
            ...
            for molecule_name in args:
                if molecule_name is None:
                    print(f'{molecule_name} is an unknown molecule.')
                    continue
                
                watermarked_image = helpers.gsrs(molecule_name)  # Assuming this function returns an Image object as BytesIO

                # Check if watermarked_image is a BytesIO and convert it to an Image
                if isinstance(watermarked_image, io.BytesIO):
                    watermarked_image.seek(0)  # Move to the beginning of the BytesIO object
                    watermarked_image = Image.open(watermarked_image)  # Convert BytesIO to PIL Image

                # Save the watermarked image
                save_image(watermarked_image, f'{molecule_name}_watermarked.png')
                view_image(watermarked_image, f'Watermarked Image for {molecule_name}')
            ...
            print('Invalid option. Use `compare`, `glow`, `gsrs`, or `shadow`.')
...
if __name__ == "__main__":
    option = input("Enter option (compare, glow, gsrs, shadow): ").strip()
