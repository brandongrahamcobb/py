''' helpers.py The purpose of this program is to be the backbone for a majority of the logic for my python projects.
...
path_config_yaml = join(path_home, '.config', 'TEMP', 'config.yaml')
path_log = join(path_home, '.log', 'discord.log')
path_users_yaml = join(path_home, '.config', 'TEMP', 'users.yaml')

def add_watermark(image: BytesIO, watermark_text: str = '~TEMP~') -> BytesIO:
