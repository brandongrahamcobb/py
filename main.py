...
class MemberData:
    ...
    def get_exp_needed_for_level(self, level: int) -> int:
        exp = [1, 15, 34, 57, 92, 135, 372, 560, 840, 1242, 1144, 1573, 2144, 2800, 3640, 4700, 5893, 7360, 9144, 11120, 
               ...
               1697021059]
        return 2000000000 if level > 200 else exp[level]

    def get_current_level(self):
        current_level = 0
        while self.experience >= self.get_exp_needed_for_level(current_level):
            current_level += 1
...
class ScriptCog(commands.Cog):
    ...
    async def on_member_join(self, member: discord.Member):
        self.member_data[member.id] = {'join_time': discord.utils.utcnow()}
        print(f'{member} has joined the server.')

    @commands.Cog.listener()
    async def on_member_remove(self, member: discord.Member):
        if member.id in self.member_data:
            del self.member_data[member.id]
        print(f'{member} has left the server.')

    @commands.Cog.listener()
    async def on_command(self, ctx: commands.Context):
        if ctx.author.id in self.member_data:
            exp_points = len(ctx.message.content) - 1  # Calculate experience based on command length
            self.member_data[ctx.author.id]['exp'] = self.member_data[ctx.author.id].get('exp', 0) + exp_points
